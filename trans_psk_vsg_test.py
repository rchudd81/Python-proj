def close_spectrum_plot():
    try:
        ...
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
            s.connect((HOST, PORT))
            # Send a shutdown header: sample_rate=0, N=0
            import struct
            s.sendall(struct.pack('dI', 0.0, 0))
    except Exception:
        pass
def update_spectrum_plot(signals, sample_rate):
    try:
        ...
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
            try:
                s.connect((HOST, PORT))
                header = struct.pack('dI', sample_rate, N)
                s.sendall(header)
                s.sendall(iq.astype(np.complex64).tobytes())
                ...
                print('Plot window not running. Launched new plot window. Please try again in a moment.')
    except Exception as e:
        print(f'Failed to update spectrum plot: {e}')
...
def main():
    ...
    while True:
        ...
        if choice == '1':
            ...
            print('Displaying/updating real-time spectrum plot of current composite IQ...')
            try:
                iq = generate_composite_iq(signals, sample_rate)
                import socket
                import struct
                ...
                print(f'Failed to display spectrum plot: {e}')
